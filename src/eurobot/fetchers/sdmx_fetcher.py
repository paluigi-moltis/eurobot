...
SERIES_SPECS: list[SeriesSpec] = [
    ...
    SeriesSpec(
        tag="DE_10Y_YIELD",
        agency="ESTAT", flow="irt_lt_mcby_m",
        key="M.MCBY.DE",
...
def _fetch_ecb_series(spec: SeriesSpec) -> pd.Series | None:
    """Fetch a single series from ECB via sdmx1."""
    try:
        ...
        s.name = spec.tag
        logger.info("ECB: %s — %d obs [%s → %s]",
                     spec.tag, len(s), s.index[0].date(), s.index[-1].date())
        return s
...
def _fetch_eurostat_series(spec: SeriesSpec) -> pd.Series | None:
    ...
    try:
        ...
        if "OBS_VALUE" not in df.columns or "TIME_PERIOD" not in df.columns:
            logger.warning("Eurostat: %s — unexpected columns", spec.tag)
            return None
        ...
        s.name = spec.tag
        logger.info("Eurostat: %s — %d obs [%s → %s]",
                     spec.tag, len(s), s.index[0].date(), s.index[-1].date())
        return s
    except Exception as exc:
...
def fetch_all_macro() -> list[dict[str, Any]]:
    ...
    logger.info("SDMX: %d/%d series fetched successfully", len(items), len(SERIES_SPECS))
    return items
